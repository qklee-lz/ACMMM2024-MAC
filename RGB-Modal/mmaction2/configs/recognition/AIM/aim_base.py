...
model = dict(
    backbone=dict(drop_path_rate=0.2, adapter_scale=0.5, num_frames=32,
                 pretrained=None, weight_path = '/root/autodl-tmp/aim_pretrained/vit_b_clip_32frame_k400.pth'),
...
train_cfg = dict(
    type='EpochBasedTrainLoop', max_epochs=40, val_begin=20, val_interval=2)
...
optim_wrapper = dict(
    type='AmpOptimWrapper',
    optimizer=dict(
        type='AdamW', lr=3e-4, betas=(0.9, 0.999), weight_decay=1e-3),
    constructor='SwinOptimWrapperConstructor',
...
work_dir = '/root/autodl-tmp/output/aim_base'
