base# Copyright (c) OpenMMLab. All rights reserved.
...
class BaseHead(BaseModule, metaclass=ABCMeta):
    ...
    def loss_by_feat(self, cls_scores: torch.Tensor,
        ...
        labels = torch.stack(labels).to(cls_scores[0][0].device)
        ...
        if cls_scores[0][0].size() != labels.size():
            top_k_acc = top_k_accuracy(cls_scores[0][0].detach().cpu().numpy(),
                                       labels.detach().cpu().numpy(),
                                       self.topk)
            for k, a in zip(self.topk, top_k_acc):
                losses[f'top{k}_acc'] = torch.tensor(
                    a, device=cls_scores[0][0].device)
    ...
    def predict(self, feats: Union[torch.Tensor, Tuple[torch.Tensor]],
        ...
        """
        cls_scores = self(feats, **kwargs)
        return self.predict_by_feat(cls_scores[0][0], data_samples)
