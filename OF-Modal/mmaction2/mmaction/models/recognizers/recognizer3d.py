...
class Recognizer3D(BaseRecognizer):
    ...
    def extract_feat(self,
        ...
        rgb_inputs = rgb_inputs.view((-1, ) + rgb_inputs.shape[2:])
        flow_inputs = flow_inputs.view((-1, ) + flow_inputs.shape[2:])
        ...
        if test_mode:
            ...
            if self.test_cfg is not None and self.test_cfg.get(
                ...
                total_views = rgb_inputs.shape[0]
                ...
                while view_ptr < total_views:
                    rgb_batch_imgs = rgb_inputs[view_ptr:view_ptr + max_testing_views]
                    flow_batch_imgs = flow_inputs[view_ptr:view_ptr + max_testing_views]
                    feat = self.backbone(rgb_batch_imgs, flow_batch_imgs)
                    if self.with_neck:
                ...
                x = self.backbone(rgb_inputs, flow_inputs)
                if self.with_neck:
                    x, _ = self.neck(x)
