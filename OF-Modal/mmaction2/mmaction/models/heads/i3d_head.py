...
#             self.fc_cls = nn.Linear(self.in_channels, self.num_classes)
        ...
        if self.use_coarse:
#             self.fc_coarse_cls = nn.Linear(self.in_channels, 7)
...
#         normal_init(self.fc_coarse_cls, std=self.init_std)
    ...
    def forward(self, x: Tensor, **kwargs) -> Tensor:
        ...
        # # If not using ArcFace, compute the classification score
        # if not self.use_arcface:
        #     cls_score = self.fc_cls(x)
        #     return cls_score
        # # Otherwise, return the features for ArcFace loss
        # return x
...
class ArcFaceLoss(nn.Module):
    ...
    def forward(self, input_features):
        ...
        # loss = F.cross_entropy(scaled_margin_cosine, labels)
