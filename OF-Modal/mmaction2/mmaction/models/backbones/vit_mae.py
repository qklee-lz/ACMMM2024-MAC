...
class VisionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 use_mean_pooling: int = False, # lqk head
                 pretrained: Optional[str] = None,
                 # return_feat_map: bool = False,
                 return_feat_map: bool = True, # lqk head
...
# #             if 'blocks' in name:
# #                 idx = int(name.split('blocks.')[-1].split('.', 1)[0])
# #                 if idx%2 == 1 and idx >= 20:
# #                     unfreeze = True
# #                 else:
# #                     unfreeze = False
# #             elif 'fc' in name:
...
@MODELS.register_module()
class VisionTransformerTwoStream(BaseModule):
    """Two-Stream Vision Transformer for handling RGB and Flow inputs."""
    ...
    def __init__(self,
        ...
        # Create two separate Vision Transformer networks
        self.rgb_vit = VisionTransformer(
            img_size=img_size,
        ...
        self.flow_vit = VisionTransformer(
            img_size=img_size,
            patch_size=patch_size,
            in_channels=in_channels,
            embed_dims=embed_dims,
        ...
        # load weight
#         checkpoint_path = '/root/autodl-tmp/vit_b_hybrid_pt_800e_k710_ft_mmv.pth'
...
#         self.rgb_vit.load_state_dict(new_state_dict1, strict=False)
        
#         new_state_dict2 = {}
#         for key, value in checkpoint2.items():
#             new_key = key.replace('backbone.', '')
#             new_state_dict2[new_key] = value
#         self.flow_vit.load_state_dict(new_state_dict2, strict=False)
        
#         del checkpoint1, checkpoint2, new_state_dict1, new_state_dict2

        # Define a fusion layer
        # self.fusion_layer = nn.Conv3d(2 * embed_dims, embed_dims, kernel_size=1)
    ...
    def forward(self, rgb: torch.Tensor, flow: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            rgb (Tensor): The input RGB data.
            flow (Tensor): The input flow data.

        Returns:
            Tensor: The fused feature of the input samples extracted by the backbone.
        """
        rgb_feat = self.rgb_vit(rgb)
        flow_feat = self.flow_vit(flow)
        # fuse_feature = 0.65 * rgb_feat + 0.35 * flow_feat
        # flow_feat = self.rgb_vit(flow)
        fuse_feature = torch.cat((rgb_feat, flow_feat), dim=1)
        return fuse_feature
        
        # Concatenate features along the channel dimension and fuse them
...
#         rgb = self.patch_embed(rgb)[0]
#         flow = self.patch_embed(flow)[0]
...
#         if self.return_feat_map:
#             rgb = rgb.reshape(b, -1, h, w, self.embed_dims)
#             rgb = rgb.permute(0, 4, 1, 2, 3)
#             flow = flow.reshape(b, -1, h, w, self.embed_dims)
