...
class SwinTransformer3D(BaseModule):
    ...
    def __init__(
        ...
        weight_path: Optional[str] = None,  # 增加weight_path参数
        ...
        self.weight_path = weight_path  # 保存weight_path
    ...
    #     # 遍历所有基础层，将其设置为评估模式并冻结所有参数
    #     for layer in self.layers:
    #         layer.eval()
    #         for param in layer.parameters():
    #             param.requires_grad = False

    #     # 如果有额外的归一化层（根据输出索引命名），也需要冻结
    ...
    def init_weights(self) -> None:
        ...
        if self.pretrained2d:
        ...
        elif self.weight_path:  # 如果指定了权重文件路径，加载该权重
            ...
            # 处理 norm 层重命名
            if hasattr(self, 'norm3'):
                new_state_dict['norm3.weight'] = new_state_dict.pop('norm.weight', None)
                new_state_dict['norm3.bias'] = new_state_dict.pop('norm.bias', None)
            
            # 处理形状不匹配的参数
            for key in list(new_state_dict.keys()):
                if "relative_position_bias_table" in key:
                    if new_state_dict[key].shape != self.state_dict()[key].shape:
