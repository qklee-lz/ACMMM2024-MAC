...
class AnySample(BaseTransform):
    ...
    def _get_sample_clips(self, num_frames: int) -> np.ndarray:
        seg_size = num_frames / self.clip_len
        inds = []
        if not self.test_mode:
            rand_choice = random.randint(0, 2)
            for i in range(self.clip_len):
                start = math.round(seg_size * i)
                end = math.round(seg_size * (i + 1))
                inds.append(np.random.randint(start, end + 1))
                
            for i in range(self.clip_len):
                start = math.round(seg_size * i) + random.randint(0, int(seg_size))
                end = math.round(seg_size * (i + 1))
                inds.append(np.random.randint(start, end + 1))
