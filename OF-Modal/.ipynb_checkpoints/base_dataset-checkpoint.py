...
class BaseDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 pipeline_flow: List[Union[dict, Callable]] = [],
        ...
        self.pipeline_flow = Compose(pipeline_flow)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.test_mode:
            ...
            data_info_flow = data_info
            data_info_flow['filename'] = self.replace_of_keyword(data_info_flow['filename'])
            # data_flow = self.pipeline(data_info_flow)
            data_flow = self.pipeline_flow(data_info_flow)

            if data is None:
                raise Exception('Test time pipline should not get `None` '
                                'data_sample')

            # data["inputs"] = [data["inputs"], data_flow["inputs"]]
            data["inputs_flow"] = data_flow["inputs"]
            return data
        ...
        for _ in range(self.max_refetch + 1):
            ...
            # print(data_flow["inputs"].shape)
            # print(data_flow["data_samples"])
    ...
    def replace_of_keyword(self, filename):
        return filename.replace('/data/train', '/data/train_of_video')
        # if 'train' in filename:
        #     return filename.replace('train', 'train_of_video')
        # elif 'val' in filename:
        #     return filename.replace('val', 'val_of_video')
        # elif 'test' in filename:
        #     return filename.replace('test', 'test_of_video')
        # else:
        #     return filename
    ...
    #     #print(data_info)
    #     #input("?")
    #     #{'filename': '/data/qklee/MAC/MAC_2024_baseline-main/MAR/mmaction2/data/ma52/videos_train/0172_02_0016.mp4', 'label': 20, 'sample_idx': 10575, 'modality': 'RGB', 'start_index': 0}
